...
class Env(BaseClass):
    ...
    def __init__(
            ...
            reward=True, length=100, seed=None, boss=False, recover=False, footprints=True, other_agents=True,
            monster_spawn_rate=1.0, view_type='symbolic'):
        ...
        self._monster_spawn_rate = monster_spawn_rate
        self._step = 0
        self._player = None
    ...
    def reset(self):
        ...
        self._unlocked = set()
        worldgen.generate_world(self._world, self._player, self._monster_spawn_rate)
        return self._obs()
    ...
    def render(self, size=None):
        size = self._size if size is None else np.array(size)
    ...
    def _balance_chunk(self, chunk, objs):
        ...
        self._balance_object(
            chunk, objs, objects.Zombie, 'grass', 6, 0, 0.01 * self._monster_spawn_rate, 0.01,
            lambda pos: objects.Zombie(self._world, pos, self._player),
            lambda num, space: (
                0 if space < 50 else 3.5 - 3 * light, 3.5 - 3 * light))
        self._balance_object(
            chunk, objs, objects.Skeleton, 'path', 7, 7, 0.01 * self._monster_spawn_rate, 0.01,
