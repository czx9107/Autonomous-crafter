...
class World:
    ...
    def __init__(self, area, materials, chunk_size, footprints=True, other_agents=True, seed=None):
        self.area = area
    ...
    def add(self, obj):
        ...
        if self._obj_map[tuple(obj.pos)] != 0:
            raise ValueError(f'Position {obj.pos} is already occupied by {self._obj_map[tuple(obj.pos)]}.')
...
class Textures:
    ...
    def __init__(self, directory):
        ...
        for filename in pathlib.Path(directory).glob('*.png'):
            ...
            if image.shape[2] < 3:
                print(f'Warning: {filename} is grayscale, converting to RGB.')
...
class LocalView:
    ...
    def _noise(self, canvas, amount, stddev):
        noise = self._world.random.uniform(
            32, 127, canvas.shape[:2])[..., None]
...
class SemanticView:
    ...
    def __call__(self):
        mat_map = self._world._mat_map.copy()
        # objs = self._world.objects
        for obj in self._world.objects:
            mat_map[tuple(obj.pos)] = self._obj_ids[type(obj)]
        return mat_map
...
class SymbolicView:
    ...
    def __init__(self, size):
        self._size = size
        self._bias = np.array(size) // 2

    def __call__(self, pos, sem_view):
        sym_view = np.zeros(self._size, np.uint8)
        for x in range(self._size[0]):
            for y in range(self._size[1]):
                pos_rel = pos + np.array([x, y]) - self._bias
                if not _inside((0, 0), pos_rel, tuple(sem_view.shape)):
                    continue
                sym_view[x, y] = sem_view[tuple(pos_rel)]
        return sym_view


class SymbolicFullView:
    ...
    def __call__(self, pos, sem_view):
        mask = np.zeros(sem_view.shape, np.uint8)
        for x in range(self._size[0]):
            for y in range(self._size[1]):
                pos_rel = pos + np.array([x, y]) - self._bias
                if not _inside((0, 0), pos_rel, tuple(sem_view.shape)):
                    continue
                mask[tuple(pos_rel)] = sem_view[tuple(pos_rel)]
        return mask
...
class DetailedView:
    ...
    def __call__(self):
        mat_map = self._world._mat_map.copy()
        mat_obj = np.zeros(self._world.area, np.uint8)
        for obj in self._world.objects:
            pos = tuple(obj.pos)
            for i, c in enumerate(self.obj_types):
                if isinstance(obj, c):
                    mat_obj[pos] = i + 1
                    break
            if mat_map[pos] == 0:
                assert (0)
        return mat_map, mat_obj
