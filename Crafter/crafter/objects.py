...
class Player(Object):
    ...
    def _sleep(self):
        if self.inventory['energy'] < constants.items['energy']['max']:
            self.sleeping = True
        last_pos = tuple(self.pos)
        material, obj = self.world[last_pos]
        if material == 'bed':
            self.sleeping_on_bed = True

    def _drink(self):
        if self.inventory['water_bucket'] > 0:
            self.inventory['water_bucket'] -= 1
            self.inventory['bucket'] += 1
            self.inventory['drink'] += 2
            last_pos = (self.pos[0], self.pos[1])
            old_material, obj = self.world[last_pos]
            land_type = old_material.split("_")[0]
            if land_type in ["grass", "sand"]:
                self.world[last_pos] = land_type + "_water"
            self._thirst = 0
    ...
    def _update_life_stats(self):
        ...
        if self.sleeping:
            if self.sleeping_on_bed:
                self._fatigue = min(self._fatigue - 2, 0)
            else:
                self._fatigue = min(self._fatigue - 0.5, 0)
        else:
            self._fatigue += 1
        if self._fatigue < -10:
            self._fatigue = 0
    ...
    def _degen_or_regen_health(self):
        ...
        if all(necessities):
            if self.sleeping:
                self._recover += 3 if self.sleeping_on_bed else 1.5
            else:
                self._recover += 1
        else:
        ...
        if self._recover < -15:
            self._recover = 0
            self.health -= 10
            if constants.SURVIVAL_LOG:
                for case in ["food", "drink", "energy"]:
                    if not self.inventory[case] > 0:
                        print("Died of " + case + " deprivation")
                        break
    ...
    def _eat(self, item):
        if self.inventory[item] > 0:
            ...
            self.health += constants.items[item]['health']
            self.inventory['food'] += constants.items[item]['health']
            self.achievements['eat_' + item] += 1
            # if item == "apple":
            #     print("Eat " + item)
            # if item in ("beef", "steak"):
            #     print("Eat " + item)
    ...
    def _do_object(self, obj):
        ...
        if isinstance(obj, Skeleton):
            ...
            if obj.health <= 0:
                ...
                player = obj.player
                self.world.remove(obj)
                self.world[pos] = "skeleton_dead"
        if isinstance(obj, Cow):
            if obj.health > 0:
                obj.health -= damage
                if obj.health <= 0:
                    self.inventory['beef'] += 2
                    self.achievements['collect_beef'] += 1
                    pos = obj.pos
                    self.world.remove(obj)
                    self.world[pos] = "cow_dead"
    ...
    def _do_material(self, target, material):
        ...
        for name, amount in info['receive'].items():
            if info.get("probability") and self.random.uniform() > info["probability"][name]:
                continue
    ...
    def update_trace(self):
        if self.world._footprints:
            self._update_footprints()

    def _update_footprints(self):
        last_pos = (self.pos[0], self.pos[1])
        material, obj = self.world[last_pos]
        land_type = material.split("_")[0]
        info = material.split("_")[1] if "_" in material else None
        tmp = None
        if self.facing[0] == 1:
            tmp = land_type + "_right"
        ...
        if self.facing[1] == -1:
            ...
            tmp = land_type + "_down"
        if land_type in ["grass", "sand"]:
            if "_" not in material and info not in ["water", "blood", "unknown"]:
                self.world[last_pos] = tmp
            elif material != tmp and info not in ["water", "blood", "unknown"]:
                self.world[last_pos] = land_type + "_unknown"

    def update_blood(self, pos):
        if self._boss:
            return
        last_pos = (pos[0], pos[1])
        material, obj = self.world[last_pos]
        land_type = material.split("_")[0]
        if land_type in ["grass", "sand", "path"]:
            self.world[last_pos] = land_type + "_blood"
...
class Cow(Object):
    ...
    def __init__(self, world, pos, alive=True):
        super().__init__(world, pos)
        self.health = 3 if alive else 0
...
class Zombie(Object):
    ...
    def __init__(self, world, pos, player, alive=True):
        super().__init__(world, pos)
        self.player = player
        self.health = 5 if alive else 0
        self.cooldown = 0
        ...
        if self.health > 0:
            ...
            if dist <= 1:
                if self.cooldown:
                    ...
                    self.player.update_blood(self.player.pos)
                    if constants.SURVIVAL_LOG:
                        print(f'zombie {damage} damage, health {self.player.health}, place {self.player.pos}')
                    self.cooldown = 5
...
class Skeleton(Object):
    ...
    def update(self):
        if self.health > 0:
            ...
            if dist <= 3:
                moved = self.move(-self.toward(self.player,
                                  self.random.uniform() < 0.6))
                if moved:
                    return
    ...
    def update(self):
        ...
        if obj:
            ...
            if isinstance(obj, Player):
                obj.update_blood(obj.pos)
            if obj.health <= 0:
                if isinstance(obj, Player):
                    if constants.SURVIVAL_LOG:
                        print(f'arrow 2 damage, health {obj.health}, pos {obj.pos}')
                pos = obj.pos
                self.world.remove(obj)
                if isinstance(obj, Player):
                    self.world.add(Player_bak(self.world, pos, False))
            ...
            self.world.remove(self)
            if material in ['table', 'furnace', 'bed']:
                self.world[target] = material + '_destroyed'
        else:
            self.move(self.facing)
