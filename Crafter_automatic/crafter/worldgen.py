...
def generate_world(world, player, monster_spawn_rate=1.0):
    ...
    for x in range(world.area[0]):
        for y in range(world.area[1]):
            _set_object(world, (x, y), player, tunnels, monster_spawn_rate)


def load_world(world, player, mat_map, mat_obj):
    for obj in world.objects:
        if not isinstance(obj, objects.Player):
            world.remove(obj)
    inventory = \
        {i: name for i, name in enumerate([None] + constants.materials)}
    for x in range(world.area[0]):
        for y in range(world.area[1]):
            world[x, y] = inventory[mat_map[x, y]]
            if mat_obj[x, y] == 0:
                continue
            elif mat_obj[x, y] == 1:
                world.add(objects.Player_bak(world, (x, y)))
            # elif mat_obj[x, y] == 1:
            #     # assert(0)
            #     world.add(objects.Player_bak(world, (x, y), alive=False))
            elif mat_obj[x, y] == 2:
                world.add(objects.Cow(world, (x, y)))
            elif mat_obj[x, y] == 3:
                world.add(objects.Cow(world, (x, y), alive=False))
            elif mat_obj[x, y] == 4:
                world.add(objects.Zombie(world, (x, y), player))
            elif mat_obj[x, y] == 5:
                world.add(objects.Zombie(world, (x, y), player, alive=False))
            elif mat_obj[x, y] == 6:
                world.add(objects.Skeleton(world, (x, y), player))
            elif mat_obj[x, y] == 7:
                world.add(objects.Skeleton(world, (x, y), player, alive=False))
            else:
                assert(0)
...
def _set_material(world, pos, player, tunnels, simplex):
    ...
    if start > 0.5:
        ...
        if simplex(x, y, 5, 7) > 0 and uniform() > 0.8:
            world[x, y] = 'tree' if random.random() > 0.2 else 'apple_tree'
        else:
            world[x, y] = 'grass'


def _set_object(world, pos, player, tunnels, monster_spawn_rate):
    x, y = pos
    uniform = world.random.uniform
    ...
    if material not in constants.walkable:
        ...
        world.add(objects.Cow(world, (x, y)))
    elif not world._other_agents:
        pass
    elif dist > 10 and _spawn_with_rate(uniform, 0.007, monster_spawn_rate):
        world.add(objects.Zombie(world, (x, y), player))
    elif material == 'path' and tunnels[x, y] and _spawn_with_rate(uniform, 0.05, monster_spawn_rate):
        world.add(objects.Skeleton(world, (x, y), player))


def _spawn_with_rate(uniform, base_probability, rate):
    return uniform() < min(1.0, max(0.0, base_probability * rate))
